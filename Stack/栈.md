...
- **递归工作栈** ：递归调用时的函数调用栈。
  ...
  - 每退出一层递归，就从栈顶弹出相应信息
  - 缺点：效率低，太多层递归可能导致栈溢出；可能包含很多重复计算。
...
2. 再定义顺序栈 **结构体** 。包括存储数字的 **数组** 和 **栈顶指针**
    ```
    // 顺序栈结构体
    struct SqStack {
...
4. **用栈模拟递归计算阶乘算法**
   1. 先判断输入是否合法
   2. 当输入为 0 时，直接返回 0 的阶乘为 1
   3. 声明并初始化一个顺序栈
   4. 把 n, n-1, n-2, ..., 2, 1 依次入栈
   5. 从栈顶开始依次出栈并相乘
   6. 返回最终计算结果
    ```
    // 用栈模拟递归计算阶乘
...
4. **使用顺序栈求斐波那契数列的算法**
   1. 先进行输入合法性判断。输入不能为负数。
   2. 最初两项是基础情况，分别为 0 和 1
   3. 声明并初始化顺序栈
   4. 设置一个 n+1 大小的数组，用来存储斐波那契数列的结果。数组内容初始化 -1 ，表示未计算
   5. 将数组的前两项设为 0 和 1 ，为基础情况
   6. 把初始值 n 入栈
   7. 弹出栈顶元素，先看该元素是否已计算过，如果计算过就跳过，如果没有计算过就检查 栈顶元素-1 和 栈顶元素-2 是否计算过。没有计算过的就入栈，都计算过了就加起来计算栈顶元素。
   8. 重复上述步骤，直到栈为空
    ```
    // 使用顺序栈求斐波那契数列
