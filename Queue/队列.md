...
## 链式队列（带头结点）的实现
1. 定义链式队列（带头结点）的 **结点结构** 。包括存储结点数据的 **数据域** 和指向下一个结点的 **指针域**
    ```
    // 链式队列（带头结点）结点结构
    struct QueueNode {
        int data;  // 数据域
...
2. 定义链式队列（带头结点）的 **队列结构** 。包括指向头结点的 **队头指针** 、指向队尾结点的 **队尾指针** 、**队列长度**
    ```
    // 链式队列（带头结点）队列结构
    struct LinkedQueue {
...
3. **初始化**
   1. 传入 **队列名 &Q**
   2. 新建一个头结点
   3. 把头结点的 next 指向空，因为当前队列中没有任何结点
   4. 再把队尾指针也指向头结点
   5. 把队列长度设为 0
    ```
    // 初始化
...
4. **判队空**
   1. 传入 **队列名 Q**
   2. 看队头指针和队尾指针是否相等，如果相等，则队列为空，如果不相等，则队列不为空
    ```
    // 判队空
...
5. **入队** （插入到队尾）
   ...
   2. 新建一个结点
   3. 把 e 的值赋给新结点
   4. 把新结点的 next 指向空，因为新结点需要插入到队尾
   5. 把新结点链接到队尾，就是把当前队尾结点的 next 指向新结点
   6. 更新队尾指针指向新结点
   7. 更新队列大小
    ```
    // 入队操作
...
6. **出队** （队首元素出队）
   1. 传入 **队列名 &Q** 、**返回值 e**
   2. 先判断队列是否为空
   3. 新建一个指针 p ，指向队首结点（不是头结点）
   4. 用 e 返回队首结点的值
   5. 更新队首指针
   6. 如果删除的是最后一个结点，即队首指针和 p 相等时，就还要更新队尾指针等于队首指针
   7. 删除队首结点 p
   8. 更新队列长度减一
    ```
    // 出队操作（队首元素出队）
...
## 队列在操作系统中的应用
- 可以用 **队列** 实现 **FCFS（先来先服务）** 策略，可以用于处理多个进程争抢使用有限的系统资源。解决由多用户引起的资源竞争问题。

- **缓冲区** 逻辑结构。解决主机与外部设备之间速度不匹配的问题。例如主机与打印机之间设置一个打印数据缓冲区。